...
public class AdminController {
    ...
    @Autowired
    public AdminController(UserRegistrationService uRegService, UserService uService, RegistrationFormCheck rFormCheck, VolunteerService vService, RewardService rService, BusinessService bService, VolunteerApplicantsService vAppsService, BusinessServiceJdbc bServiceJ) {
    ...
    private String searchForReward(Model model, OfferSearchForm oForm) {
        ...
        List<Reward> matchingRewards = this.rService.findRewardsBySearch(oForm.getSearchNameTerm());
        LOG.debug(matchingRewards.toString());
        model.addAttribute("offers", matchingRewards);

        List<String> businesses = new ArrayList<String>();
        for (Reward o : matchingRewards) {
    ...
    public String deleteReward(@PathVariable Long id, Model model) {
        ...
        try {
            Reward rewardToDelete = this.rService.findById(id);
            LOG.debug(rewardToDelete.toString());
        } catch (EmptyResultDataAccessException e) {
            //0 rows returned - reward doesn't exist in database table.
            String msg = "The reward you're looking for does not exist.";
            LOG.debug(msg);
        ...
        Integer resultOfRemoval = this.rService.deleteByRewardID(id);
        LOG.debug("Rows affected by removal: " + resultOfRemoval);

        if (resultOfRemoval == 0) {
            String msg = "Reward could not be deleted.";
            model.addAttribute("issue", msg);
            return "404";
        } else {
            model.addAttribute("content", "adminSearchRewards");
    ...
    @RequestMapping(path = "/enableReward/{id}", method = RequestMethod.GET)
    public String enableReward(@PathVariable Long id, Model model) {
        LOG.debug("Handling /enableReward/{id}");
        LOG.debug(id.toString());
        try {
            Reward rewardToEnable = this.rService.findById(id);
            LOG.debug(rewardToEnable.toString());
        } catch (EmptyResultDataAccessException e) {
            return "404";
        }

        LOG.debug("Enabling reward");
        Integer result = this.rService.enableByRewardID(id);
    ...
    public String orderRewardsByTotal(RewardStatisticsForm rStatForm, Model model) {
        ...
        if (rStatForm.getOrderingChoice().equals("total")) {
            ...
            List<Reward> rewards = this.rService.findAllByTotal();
            ...
            List<Reward> rewards = this.rService.findAllAlphabetically();
            LOG.debug(rewards.toString());
    ...
    public String createNewReward(CreateRewardForm rForm, Model model) {
        ...
        Reward reward = new Reward(rForm.getName(), rForm.getDescription(), rForm.getCost(), businessID);

        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (!(authentication instanceof AnonymousAuthenticationToken)) {
            String currentUserName = authentication.getName();

            Integer rowsAffected = this.rService.createNewOffer(reward, currentUserName);
            LOG.debug("Rows affected by insertion: " + rowsAffected);

            model.addAttribute("rwrd", reward);
