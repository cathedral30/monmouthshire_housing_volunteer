...
public class UserProfileController {
    ...
    private RewardService oService;
    private AchievementDetailsService aService;
    private BusinessService bService;

    @Autowired
    public UserProfileController(RewardService oService, RedemptionService rService, VolunteerService vService, AchievementDetailsService aService, BusinessService bService) {
    ...
    private String serveCreditHistoryForUser(@SessionAttribute("user") VolunteerUser user, Model model) {
        ...
        for (Redemption r : usersCreditHistory) {
            try {
            redeemRewards.add(new RedeemReward(r, this.oService.findById(r.getReward_id())));
            } catch (Exception e) {
    ...
    public String viewOffers(Model model) {
        ...
        List<Reward> listOfRewards = oService.findAll();
        LOG.debug(listOfRewards.toString());
        List<RewardAndOffer> rewardAndOffers = new ArrayList<>();
        for (Reward reward : listOfRewards) {
            try {
            rewardAndOffers.add(new RewardAndOffer(this.bService.findBusinessNameById(reward.getBusinessID()), reward));
            } catch (Exception e) {
                LOG.debug(e.toString());
            }
        }
